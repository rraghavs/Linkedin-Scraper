...
@app.route("/")
def index():
    return render_template("test_2 (1).html")
...
def livesearch():
    ...
    query1 = "SELECT Level FROM tabel WHERE Job_skills like %s  GROUP BY Level ORDER BY COUNT(*) DESC LIMIT 1"
    cursor.execute(query1, (searchbox,))
    experience_level = cursor.fetchone()

    # Query 2: Most common industry where the skill is required
    query2 = "SELECT Industry FROM tabel WHERE Job_skills like %s GROUP BY Industry ORDER BY COUNT(*) DESC LIMIT 1"
    cursor.execute(query2, (searchbox,))
    industry = cursor.fetchone()

    # Query 3: Most common Company Class where the skill is required
    query3 = "SELECT Class FROM tabel WHERE Job_skills like %s GROUP BY Class ORDER BY COUNT(*) DESC LIMIT 1"
    cursor.execute(query3, (searchbox,))
    company_class = cursor.fetchone()

    # Query 4: Number of Jobs available for the skill
    query4 = "SELECT COUNT(*) AS num_jobs FROM tabel WHERE Job_skills like %s"
